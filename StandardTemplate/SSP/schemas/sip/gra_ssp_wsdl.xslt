<xsl:stylesheet version="2.0" 
	...
	<xsl:template match="/">
		<xsl:for-each select="/graa:ServiceDescription/serviceInterface">			
			<xsl:variable name="fileLocation" select="lower-case(concat((serviceInteractionProfile/@sipName,'SIP_WS_')[1], (serviceInteractionProfile/@sipVersion,'1.3')[1], '/', (@serviceInterfaceNameAbbreviationText,'webservice')[1], '.wsdl'))"/>	
			<xsl:result-document href="{$fileLocation}">
							...
							<wsdl:portType name="{@name}">
								...
								<xsl:for-each select="interface">
									<xsl:for-each select="operation">
										<xsl:variable name="operationName"><xsl:value-of select="@actionName"/></xsl:variable>
											...
											<xsl:element name="wsdl:operation">
												...
												<xsl:element name="soap:operation">
													<xsl:attribute name="soapAction">
														<xsl:value-of select="concat(../../../../@targetNamespace, '/', ../@name, '/', @actionName)"/>
													</xsl:attribute>
													<xsl:attribute name="style">document</xsl:attribute>
												</xsl:element>												
												<xsl:for-each select="parameter[(@use = 'in') and (not(message/@messageLocationCode) or (message/@messageLocationCode != 'header'))][1]">
								...
								<xsl:choose>
									<xsl:when test="interface/@bindingCode = 'httpGet'">
										<xsl:element name="http:address">
											<xsl:attribute name="location">
												<xsl:if test="@addressUri">
													<xsl:value-of select="@addressUri"/>
												</xsl:if>
												<xsl:if test="not(@addressUri)">
