...
final class NIEMmpdmodel2artifactDelegate extends NIEMpsm2xsdDelegate {
	private static final Logger LOG = LoggerFactory.getLogger(NIEMmpdmodel2artifactDelegate.class);// tjd added LOG
    ...
    protected NIEMmpdmodel2artifactDelegate(final URI selection) {
        super(selection, org.ijis.gra.uml.qvt.Activator.INSTANCE.getNIEMmpdmodel2artifactLocation(), Activator.INSTANCE
    ...
    protected final void persist() throws IOException {
        ...
        if (!theChangelog.exists()) {
            ...
            // tjd added test
            if(theMPD==null){
            	LOG.error("Failed to locate PSM MPD in "+theSelectedResource);
            }else
            ...
            if (isExchangeSchemaSet(fileSet)) {
                ...
                for (final FileType file : files) {
                    if (isExchangeSchema(file)) {
                    	// tjd added test
                    	XSDSchema test=findSchemaFor(file);
                    	if(test!=null)
                            theExchangeSchemas.add(test);
