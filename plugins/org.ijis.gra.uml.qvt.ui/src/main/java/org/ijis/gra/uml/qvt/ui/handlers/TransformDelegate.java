...
//  private static final String NIEM_XSD_LIBRARY = "platform:/plugin/" + org.search.niem.xsd.library.Activator.SCHEMA_FOLDER;
  private static final String NIEM_XSD_LIBRARY = "platform:/plugin/" + org.ijis.gra.xsd.library.Activator.SCHEMA_FOLDER;
  protected final Collection<Resource> infrastructureResources = new ArrayList<>(4);
...
*/    	
    ...
    protected String getXmlSchemasFolder() {
    	// tjd TODO: inaccessible
        //return Activator.getPlugin().getPreferenceStore().getString(P_XML_SCHEMAS_FOLDER);
        return "XMLschemas";
    }
    protected Resource getInfrastructureResource(final URI uri, final URI targetFolder) {
        final URI oldPrefix = URI.createURI(NIEM_XSD_LIBRARY);
        final URI newLocation = uri.replacePrefix(oldPrefix, targetFolder);
        //LOG.info("getInfrastructureResource "+newLocation+", "+uri+", "+oldPrefix+", "+targetFolder+", "+oldPrefix.isPrefix()+", "+oldPrefix.authority()+", "+oldPrefix.scheme()+", "+oldPrefix.device());
